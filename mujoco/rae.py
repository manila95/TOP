...
class RAE_Agent:
    ...
    def __init__(
        ...
        self.qc_funcs = QuantileDoubleQFunc(state_dim, action_dim, n_quantiles=n_quantiles, hidden_size=hidden_size).to(device)

        self.target_q_funcs = copy.deepcopy(self.q_funcs)
        self.target_qc_funcs = copy.deepcopy(self.qc_funcs)

        self.target_q_funcs.eval()
        self.target_qc_funcs.eval()

        for p in self.target_q_funcs.parameters():
            p.requires_grad = False

        for p in self.target_qc_funcs.parameters():
            p.requires_grad = False
        ...
        self.qc_optimizer = torch.optim.Adam(self.qc_funcs.parameters(), lr=lr)
    ...
    def update_target(self) -> None:
        ...
        with torch.no_grad():
            ...
            for target_q_param, q_param in zip(self.target_qc_funcs.parameters(), self.qc_funcs.parameters()):
                target_q_param.data.copy_(self.tau * q_param.data + (1.0 - self.tau) * target_q_param.data)
    ...
    def update_q_functions(
        ...
        beta: torch.Tensor) -> [torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    def update_qc_functions(
        ...
        with torch.no_grad():
            ...
            nextaction_batch.clamp_(-self.action_lim, self.action_lim)
            # get quantiles at (s', \tilde a)
            quantiles_t1, quantiles_t2 = self.target_qc_funcs(nextstate_batch, nextaction_batch)
            # compute mean and std
            quantiles_all = torch.stack([quantiles_t1, quantiles_t2], dim=-1) # [batch_size, n_quantiles, 2]
            mu = torch.mean(quantiles_all, axis=-1) # [batch_size, n_quantiles]
            # compute std by hand for stability 
        ...
        # get quantiles at (s, a)
        quantiles_1, quantiles_2 = self.qc_funcs(state_batch, action_batch)
        # compute pairwise td errors
        td_errors_1 = quantile_target - quantiles_1[..., None] # [batch_size, n_quantiles, n_quantiles]
        td_errors_2 = quantile_target - quantiles_2[..., None] # [batch_size, n_quantiles, n_quantiles]
        # compute quantile losses
        ...
        return loss_1, loss_2, quantile_target


    def update_policy(self, state_batch: torch.Tensor, beta: torch.Tensor) -> torch.Tensor:
        """update the actor. 
    ...
    def optimize(
        ...
        """
        q1_loss, q2_loss, qc1_loss, qc2_loss, wd, pi_loss = 0, 0, 0, 0, 0, None
        for i in range(n_updates):
            ...
            cost_batch = torch.FloatTensor(samples.cost).to(device).unsqueeze(1)


            # update qc
            qc1_loss_step, qc2_loss_step, qc_target = self.update_qc_functions(state_batch, action_batch, cost_batch, nextstate_batch, done_batch, 1)
            qc_loss_step = qc1_loss_step + qc2_loss_step

            # Set beta relative to base optimism 
            base_beta = torch.empty_like(cost_batch).fill_(beta)
            rae_beta = base_beta - qc_target.squeeze().mean(axis=-1).unsqueeze(-1)
            
            # update q-funcs
            q1_loss_step, q2_loss_step, quantiles1_step, quantiles2_step = self.update_q_functions(state_batch, action_batch, reward_batch, nextstate_batch, done_batch, rae_beta)
            ...
            qc2_loss += qc2_loss_step.detach().item()
            qc1_loss += qc1_loss_step.detach().item()
            ...
            if self._update_counter % self.update_interval == 0:
                ...
                for p in self.q_funcs.parameters():
                    p.requires_grad = False
                pi_loss_step = self.update_policy(state_batch, rae_beta)
        ...
        return q1_loss, q2_loss, qc1_loss, qc2_loss, pi_loss, wd / n_updates, quantiles1_step, quantiles2_step, torch.mean(rae_beta).item()
